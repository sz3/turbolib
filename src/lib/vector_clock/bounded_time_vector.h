...
public:
	...
	bounded_time_vector(const std::deque<clock>& clocks)
		: _clocks(clocks)
	{
	}
	...
	bool operator<(const bounded_time_vector& other) const
	{
		return compare(other) == LESS_THAN;
	}

	COMPARE compare(const bounded_time_vector& other) const
	{
	...
	void merge(const bounded_time_vector& other)
	{
		...
		for (typename std::deque<clock>::const_iterator it = other._clocks.begin(); it != other._clocks.end(); ++it)
		{
			...
			else if (current->clockLessThan(*it))
			{
				_clocks.erase(current);
				_clocks.insert(_clocks.begin()+prevIndex++, *it);
			}
			else //if (current.clockGreaterThanEqual(*it))
			{
...
protected:
	typename std::deque<clock>::const_iterator findElem(const std::deque<clock>& clocks, const clock& elem) const
	{
		...
		if (it != clocks.end() && it->clockLessThanEquals(elem))
			return it;
		return clocks.end();
	}
	...
	unsigned compare(const std::deque<clock>& lesser, const std::deque<clock>& greater) const
	{
		...
		for (typename std::set<clock>::const_iterator it = first.begin(); it != first.end(); ++it)
		{
			...
			if (it->clockLessThan(*other))
				result |= LESS_THAN;
			else if (other->clockLessThan(*it))
				result |= GREATER_THAN;
			second.erase(other);
		}
		if (!second.empty())
